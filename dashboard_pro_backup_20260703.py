...
# ============ إعدادات الاتصال ============
WAZUH_API_URL = "https://192.168.1.9:55000"
WAZUH_USER = "wazuh-wui"
WAZUH_PASS = "wazuh-wui"

OPENSEARCH_URL = "https://192.168.1.9:9200"
...
def search_alerts(technique_id, minutes=5, rule_id=None):
    """Search for alerts in OpenSearch - يبحث عن rule.id و mitre.id"""
    ...
    # إذا كان rule_id محدد، ابحث عنه أولاً
    if rule_id:
        must_conditions.append({"term": {"rule.id": rule_id}})
    else:
        # fallback: ابحث عن mitre.id
...
def generate_pdf_report(results, coverage, scan_time):
    ...
    class PDFReport(FPDF):
        ...
        def footer(self):
            ...
            self.cell(0, 10, f'Page {self.page_no()}/{{nb}} | Confidential - For Internal Use Only', 0, 0, 'C')
    ...
    pdf.ln(5)
    if coverage >= 80:
        assessment = "STRONG - SOC detection capabilities are effective"
        color = (34, 197, 94)
    elif coverage >= 60:
        assessment = "MODERATE - Some gaps need attention"
        color = (234, 179, 8)
    else:
        assessment = "CRITICAL - Significant detection gaps"
        color = (239, 68, 68)
    ...
    recommendations = [
        "Review and enhance Wazuh detection rules for missed techniques",
        "Enable Sysmon on Windows agents for deeper visibility",
        "Implement additional correlation rules for multi-stage attacks",
        "Conduct regular Purple Team exercises to maintain detection readiness",
...
with col1:
    ...
    <div class='metric-card'>
        ...
        <div style='font-size: 2.5rem; font-weight: 800; color: #3b82f6;'>{}</div>
        <div style='color: #64748b; font-size: 0.85rem;'>MITRE ATT&CK</div>
    </div>
    """.format(len(ATTACKS_DB)), unsafe_allow_html=True)

with col2:
    st.markdown("""
    <div class='metric-card'>
        <div style='color: #94a3b8; font-size: 0.85rem;'>TACTICS COVERED</div>
        <div style='font-size: 2.5rem; font-weight: 800; color: #8b5cf6;'>{}</div>
        <div style='color: #64748b; font-size: 0.85rem;'>Attack Categories</div>
    </div>
    """.format(len(set(a['tactic'] for a in ATTACKS_DB.values()))), unsafe_allow_html=True)
...
if run_full:
    ...
    for idx, (attack_id, attack) in enumerate(ATTACKS_DB.items()):
        ...
        # استخدام technique_id بدلاً من expected_rule
...
if 'results' in st.session_state and st.session_state['results']:
    ...
    tactic_summary = df.groupby('tactic').agg({
        'detected': ['sum', 'count']
    }).reset_index()
    ...
    fig = px.bar(tactic_summary, x='Tactic', y='Coverage',
                 ...
                 range_y=[0, 100])
    fig.add_hline(y=80, line_dash="dash", line_color="#3b82f6", 
...
# قائمة Clients معروفة (من Velociraptor filestore)
...
with edr_col1:
    ...
    for idx, (stat_name, cmd) in enumerate(stats_commands.items()):
        output, _ = execute_ssh(cmd, timeout=5)
        if output:
...
with edr_col2:
    ...
    if st.button("🔍 Execute Forensic", use_container_width=True, type="primary"):
        with st.spinner(f"Executing {selected_forensic} on {selected_client}..."):
            ...
            if client_os == "linux":
                output, error = execute_ssh(command, timeout=20)
                ...
                st.warning(f"⚠️ Windows forensics requires SSH/WinRM on {selected_client}")
                st.info(f"**Command to execute:**\n\n`{command}`")
    ...
    if st.button("⚡ Execute Custom Command", use_container_width=True):
        if custom_command:
            with st.spinner("Executing..."):
                output, error = execute_ssh(custom_command, timeout=30)
...
with adv_col2:
    ...
    if st.button("🔎 Run Threat Check", use_container_width=True):
        with st.spinner(f"Running {selected_threat}..."):
            output, _ = execute_ssh(threat_checks[selected_threat], timeout=20)
...
<div style='text-align: center; padding: 30px; color: #64748b;'>
    <p style='font-size: 1.1rem; margin: 5px 0;'>🛡️ <strong>R&M PurpleOps Pro v3.0</strong></p>
    <p style='font-size: 0.9rem; margin: 5px 0;'>Automated Purple Team Platform</p>
    <p style='font-size: 0.8rem; margin: 10px 0;'>Powered by Python + Wazuh + OpenSearch + MITRE ATT&CK</p>
